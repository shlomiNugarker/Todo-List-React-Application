  ...
  return (
    <>
      <div
        id="popup-modal"
        tabIndex={-1}
        className={`fixed top-0 right-0 left-0 z-50 flex justify-center items-center w-full h-[calc(100%-1rem)] max-h-full overflow-y-auto overflow-x-hidden ${animationClass}`}
      >
        <div className="relative p-4 w-full max-w-md max-h-full">
          <div
            className={`relative rounded-lg shadow bg-gray-800 ${animationClass}`}
          >
            <button
              type="button"
      ...
      <div
        onClick={handleClose}
        className={`fixed inset-0 z-40 bg-slate-900 bg-opacity-50 backdrop-blur-sm ${animationClass}`}
      ></div>
    </>
  );
};
